...
my $USAGE =<<USAGE;
	...
	[example]
	perl Gfusion.pl -o out_file -r 0 -p 6 -index /path/to/bowtie1_index -gtf /path/to/genes.gtf -1 test_1.fastq -2 test_2.fastq
	...
	-2		pair-end_fastq_2
	-fq		single-end_fastq
	-r		inner distance between mate pairs
	-p		thread
	-n		min split num
	-pos_length	the anchor length
	-intron_length	min distance between adjacent genes
	-index		bowtie_index
...
#//-----------------------------------//
	...
	GetOptions(
		...
		"n:i"=>\$split_num,
		"p:i"=>\$thread,
		'index=s' => \$bowtie_index,
		'gtf=s' => \$opt_gtf,
		'help!' => \$opt_help,
		"pos_length:i"=>\$pos_length,
	...
	if (!$bowtie_index ) { print " warning: missing parameters -index!","\n"; exit 0; }

	
	if (!$opt_gtf ) { print "URO warning: missing parameters -gtf!","\n"; exit 0; }

	
	
#	if ( !$ARGV[0] ) { print "URO warning: missing unmapped.sam file!","\n";exit 0;}
	if($fastq_1) {
		...
		if($segment_length <10 or $pos_length<0) { print "error about pos_length!","\n"; exit 0; }
	...
	if($fastq_1 and $fastq_2 and !$fastq) {
		...
		&result("$out_file/temp","$fusion_out/final/map.sam","$out_file/result");
...
=cut
		...
		&single_result("$out_file/temp","$fusion_out/final/map.sam","$out_file/result");
		...
		print STDERR " Completed successfully!  The time elapsed: about ",0.01*int ( (time()-$start_when)/36 )," hours.\n";			
		 ...
		 print "URO warning: missing fastq file!","\n"; exit 0;
	}
...
sub seed_find { 
	...
	while($pos=<POS>) {
		...
		$pos =~ s/\s+($pos[7])//;
		print SRR $1,"\n";
		if(exists $split{$pos}) {   #基因和坐标重复
			$split{$pos}++;		 #==========添加数量	$srr{$pos} = $srr{$pos}.$srr."\n";
		} else {
			$split{$pos} = 1; #==========添加数量 	$srr{$pos} = $sr."\n";
			$gene = $pos[0]."\t".$pos[3];		#只计算基因重复
